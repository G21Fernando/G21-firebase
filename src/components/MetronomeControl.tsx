...
const MetronomeControl: React.FC<MetronomeControlProps> = ({ onPointsUpdate, onPracticeTimeUpdate }) => {
  ...
  return (
    <div className="p-3 md:p-10">
      <Button 
        className="w-full mb-2 md:mb-10 bg-[#1A1F2C] hover:bg-[#2A2F3C] text-white text-base md:text-xl py-3 md:py-7 rounded-xl"
        onClick={isPlaying ? stopMetronome : startMetronome}
      >
        {isPlaying ? 'Stop' : 'Start'}
      </Button>

      <div className="flex flex-col items-center gap-2 md:gap-8 mb-2 md:mb-8">
      ...
      {isPlaying && (
        <div className="text-center mt-2 md:mt-8 text-base md:text-xl font-medium text-[#1A1F2C]">
